...
class ROS_env:
    ...
    def reset(self, is_transform_available):
        ...
        self.cmd_vel_publisher.publish_cmd_vel(
            linear_velocity=1.0, angular_velocity=action[0]
        )
        ...
        latest_scan, latest_position, collision, action, reward, free_pixels= self.step(
                is_transform_available, lin_velocity=0.0, ang_velocity=action[0]
            )
    ...
    def get_reward(self, collision, action, laser_scan):
        ...
        if collision:
            ...
            totol_reward = map_reward- collision_penalty- idling_penalty
            ...
            reward_str += f"Ang vel: {action[1]:.2f} " 
            print(reward_str)
            return totol_reward
