...
def main(args=None):
    ...
    action_dim = 1  # number of actions produced by the model
    max_action = 1 # maximum absolute value of output actions
    state_dim = 21  # number of input values in the neural network (vector length of state input)
    device = torch.device(
        "cuda" if torch.cuda.is_available() else "cpu"
    )  # using cuda if it is available, cpu otherwise

    # Resource saving parameters
    nr_eval_episodes = 1  # how many episodes to use to run evaluation
    ...
    while epoch < max_epochs:  # train until max_epochs is reached
        ...
        a_in = np.array([linear_vel, action[0]])  
        ...
        if (
            ...
            eval(
                ...
                epoch=epoch,
                max_steps=max_steps,
                linear_vel=linear_vel)  # run evaluation
    plot_map_value(map_value)
    print(f"Number of collisions : {num_collisions}")

def eval(model, env, scenarios, epoch, max_steps, linear_vel):
    ...
    for scenario in scenarios:
        count = 0
        latest_scan, robot_odom, collision, goal, a, reward = env.eval(
            scenario=scenario
        )
        while count < max_steps:
            ...
            a_in = np.array([linear_vel, action[0]])  
            latest_scan, latest_position, collision, action, reward, free_pixels= env.step(
