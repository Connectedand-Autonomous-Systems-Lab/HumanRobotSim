...
class SAC(object):
    ...
    def __init__(
        ...
        save_directory=Path("src/DRL-exploration/furthest_dir/models/SAC"),
        model_name="SAC",
        load_directory=Path("src/DRL-exploration/furthest_dir/models/SAC"),
    ):
        super().__init__()
    ...
    def update_actor_and_alpha(self, obs, step):
        # print("in update actor and alpha")
        dist = self.actor(obs)
        action = dist.rsample()
    ...
    def prepare_state(self, latest_scan, odom, cos, sin, collision, action):
        # update the returned data from ROS into a form used for learning in the current model
        latest_scan = np.array(latest_scan)

        # Define a threshold for "too large" numbers
        large_threshold = 1000 # Adjust based on your needs

        # Create a mask for inf values
        inf_mask = np.isinf(latest_scan)

        # Create a mask for very large values
        large_value_mask = np.abs(latest_scan) > large_threshold

        # Combine both masks
        invalid_mask = inf_mask | large_value_mask

        # Replace invalid values with 7.0
        latest_scan[invalid_mask] = 50.0

        # print(latest_scan)
        main_states_dim = 4        # the odom2 action2
        max_bins = self.state_dim - main_states_dim   # 19
        bin_size = int(np.ceil(len(latest_scan) / max_bins))      #  where len(latest scan is 360)
        # bin size is 19
        # print(f"laser scan {len(latest_scan)}")
        ...
        # len(min_values) is 18
        # print(f"min values {len(min_values)}")
        # print(min_values)
        state = min_values + [cos, sin] + [action[0], action[1]]
        print(state)
        # print(f"cos: {cos:.2f} | sin: {sin:.2f}")
