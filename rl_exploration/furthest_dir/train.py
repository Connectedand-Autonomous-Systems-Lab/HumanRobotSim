...
def main(args=None):
    ...
    latest_scan, robot_odom, cos, sin, collision, a, reward, free_pixels = ros.reset(is_transform_available)
    map_value = []
    num_collisions = 0
    ...
    while epoch < max_epochs:  # train until max_epochs is reached
        ...
        state, terminal = model.prepare_state(
            latest_scan, robot_odom, cos, sin, collision, a
        ...
        # print(a_in[0],a_in[1])
        latest_scan, robot_odom, cos, sin, collision, a, reward, free_pixels = ros.step(
        ...
        if (
            ...
            latest_scan, robot_odom, collision, cos, sin, a, reward, free_pixels = ros.reset(is_transform_available)
            steps = 0
        else:
            steps += 1
...
def eval(model, env, scenarios, epoch, max_steps):
    ...
    for scenario in scenarios:
        count = 0
        latest_scan, robot_odom, cos, sin, collision, goal, a, reward = env.eval(
            scenario=scenario
        )
        while count < max_steps:
            ...
            latest_scan, latest_position, cos, sin, collision, action, reward, free_pixels= env.step(
