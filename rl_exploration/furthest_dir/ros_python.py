...
class ROS_env:
    ...
    def step(self, is_tf_available, lin_velocity=0.0, ang_velocity=0.1):
        ...
        cos, sin = self.get_furthest_angle(latest_scan,0,2*math.pi)
        reward = self.get_reward(collision, action, latest_scan)

        return latest_scan, latest_position, cos, sin, collision, action, reward, free_pixels
    ...
    def reset(self, is_transform_available):
        ...
        self.robot_trajectory = []
        latest_scan, latest_position, cos, sin, collision, action, reward, free_pixels= self.step(
                is_transform_available, lin_velocity=action[0], ang_velocity=action[1]
            )
        return latest_scan, latest_position, cos, sin, False, action, reward, free_pixels
    ...
    def eval(self, scenario):
        ...
        return latest_scan, latest_position, cos, sin, False, False, action, reward
    ...
    def get_furthest_angle(self,scan_ranges, angle_min, angle_max):
        """
        ...
        print(max_index)
        # print(f"up :{scan_ranges[90]} right : {scan_ranges[0]} left: {scan_ranges[179]}")
        ...
        angle_degrees = np.degrees(furthest_angle)
        # print(f"angle in degrees: {angle_degrees}")
        cos_angle = np.cos(furthest_angle)
        sin_angle = np.sin(furthest_angle)
    ...
    def get_reward(self, collision, action, laser_scan):
        ...
        if collision:
            ...
            longest_facing = face_longest()
            trajectory_deviation_ = trajectory_deviation()
            totol_reward = action[0]  + map_reward- collision_penalty - ang_penalty + longest_facing
            ...
            reward_str += f"facing rew: {longest_facing:.2f} | " 
